...
export class Employees extends AirbyteStreamBase {
    constructor(readonly config: AirbyteConfig, logger: AirbyteLogger) {
        super(logger);
    }

    getJsonSchema(): Dictionary<any, string> {
        return require('../../resources/schemas/employees.json');
    }
    ...
    async* readRecords(
        ...
        do {
            const response = await axios.get(this.config.server_url + '/api/v2/hrm/employee', {
                headers: {
